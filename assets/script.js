document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(legacyRegistration && (location.pathname.endsWith('/index.html') || location.pathname==='/' || location.pathname.endsWith('/vaagai-2k26/'))){legacyRegistration.remove()}

  if(location.pathname.endsWith('/events.html') || location.pathname.endsWith('events.html')){
    const map={'Paper Presentation':'paper-presentation','CAD Modeling':'cad-modeling','ANSYS Simulation Challenge':'ansys-simulation','Glider Competition':'glider-competition','Technical Quiz':'technical-quiz','Line Follower':'line-follower','Water Rocketry':'water-rocketry','Free Fire':'free-fire','Carrom':'carrom','IPL Auction':'ipl-auction','Treasure Hunt':'treasure-hunt','Chess':'chess'};
    document.querySelectorAll('.detail-card h2').forEach(h=>{const key=map[h.textContent.trim()];if(!key||h.querySelector('a'))return;const a=document.createElement('a');a.href=`registration.html?event=${encodeURIComponent(key)}`;a.textContent=h.textContent.trim();a.setAttribute('aria-label',`Register for ${h.textContent.trim()}`);a.style.color='inherit';a.style.textDecoration='none';h.replaceChildren(a)})
  }
